...
import {
    TRACKER_ENTITY_INSTANCES_ROUTE,
    DATA_STORE_ROUTE,
} from "../api.routes";
import {
    ATTRIBUTE,
...
const drawCamember = async (
    ...
    const current_legend_parent = legendContentList?.find(
        (leg) => leg.id === legendTypeId,
    );
    const current_legend = findTheRightLegend(
        current_legend_parent,
        period,
        periodType,
    );
...
const findTheRightLegend = (currentLegendParent, period, periodType) => {
    if (!currentLegendParent) return null;

    let currentLegend = null;
    const legendSetListKey = Object.keys(currentLegendParent?.periods);

    if (periodType === YEAR) {
        const legendWithNoEndDate =
            legendSetListKey.filter((l) => (l?.split("_")[1] ? false : true)) ||
            [];
        const legendWithEndDate =
            legendSetListKey.filter((l) => (l?.split("_")[1] ? true : false)) ||
            [];

        if (
            legendWithNoEndDate?.length > 0 &&
            parseInt(dayjs(legendWithNoEndDate[0]).format("YYYY")) <=
                parseInt(dayjs(period).format("YYYY"))
        ) {
            currentLegend =
                currentLegendParent?.periods[`${legendWithNoEndDate[0]}`];
        } else {
            const filteredLegend = legendWithEndDate.reduce((prev, current) => {
                const start = parseInt(
                    dayjs(current.split("_")[0]).format("YYYY"),
                );
                const end = parseInt(
                    dayjs(current.split("_")[1]).format("YYYY"),
                );
            ...
            currentLegend =
                currentLegendParent?.periods[`${filteredLegend[0]}`];
        }
    }
    ...
    if (periodType === MONTH) {
        ...
        if (
            legendWithNoEndDate?.length > 0 &&
            (dayjs(legendWithNoEndDate[0])
                .startOf("month")
                .isBefore(dayjs(period).startOf("month")) ||
                dayjs(legendWithNoEndDate[0])
                    .startOf("month")
                    .isSame(dayjs(period).startOf("month")))
        ) {
            currentLegend =
                currentLegendParent?.periods[`${legendWithNoEndDate[0]}`];
        } else {
            const filteredLegend = legendWithEndDate.reduce((prev, current) => {
                ...
                if (
                    (dayjs(start).isBefore(dateChoosed) ||
                        dayjs(start).isSame(dateChoosed)) &&
                    (dayjs(dateChoosed).isBefore(end) ||
                        dayjs(dateChoosed).isSame(end))
                ) {
                    prev.push(current);
                }
    ...
    if (periodType === DAY) {
        ...
        if (
            legendWithNoEndDate?.length > 0 &&
            (dayjs(legendWithNoEndDate[0]).isBefore(dayjs(period)) ||
                dayjs(legendWithNoEndDate[0]).isSame(dayjs(period)))
        ) {
            currentLegend =
                currentLegendParent?.periods[`${legendWithNoEndDate[0]}`];
        } else {
...
const displayNotificationIfLegendIsNotSet = (setNotif, legendName, period) => {
    setNotif({
        show: true,
        message: `Some legends have not been configured for the selected period ( ${dayjs(
            period,
        ).format(
            "YYYY",
...
const checkLabelLegend = async (
    ...
    if (current_legend && current_legend.items?.length > 0) {
        ...
        displayNotificationIfLegendIsNotSet(
            setNotif,
            current_legend_parent?.name,
            period,
        );
    }
};

const checkColorLegend = async (
    legendTypeId,
    attribute_code,
    value,
    period,
    ...
    if (current_legend && current_legend.items?.length > 0) {
        for (let item of current_legend.items) {
            if (!item.color) {
                ...
                return setNotif({
                    show: true,
                    message: `You try to display some colors that have not been configurated for the selected period ( ${dayjs(
                        period,
                    ).format(
                        "YYYY",
                    )} ) , the values ​​will be displayed instead of these legends ! `,
...
export const inject_tei_into_html = (
    report,
    current_tei,
    selectedProgramTrackerFromHTML,
    setNotif,
) => {
    if (!selectedProgramTrackerFromHTML) return null;

    if (!current_tei) return null;

    let my_container = document.querySelector('[id="my-table-container"]');
    const report_html_cloned = report.html;

    let parser = new DOMParser();
    const report_html_cloned_document = parser.parseFromString(
        report_html_cloned,
        "text/html",
    );

    let program_tracker_list = report_html_cloned_document.querySelectorAll(
        "[data-type=" +
            TRACKER.value +
            '][id*="' +
            selectedProgramTrackerFromHTML.id +
            '"]',
    );
    ...
    for (let program_tracker of program_tracker_list) {
        ...
        const get_data_has_legend =
            program_tracker.getAttribute("data-has-legend");

        // Interprétation des données sur les attributes
        if (get_data_is === ATTRIBUTE) {
            if (get_id) {
                const get_attribute_id = get_id.split("|")?.[1];
                const attribute_found = current_tei.attributes?.find(
                    (at) => at.attribute === get_attribute_id,
                );
                const html_el = my_container.querySelector(
                    "[id='" + get_id + "']",
                );
                ...
                src="${TRACKER_ENTITY_INSTANCES_ROUTE.concat("/")
        ...
        if (get_data_is === ENROLLMENT) {
            if (get_id) {
                ...
                if (get_enrollment_html_id === ENROLLMENT_DATE) {
                    ...
                    html_el.innerHTML = current_tei.enrollments?.[0]
                        ?.enrollmentDate
                        ? dayjs(
                              current_tei.enrollments?.[0]?.enrollmentDate,
                          ).format("YYYY-MM-DD")
                        : "";
                }
                ...
                if (get_enrollment_html_id === INCIDENT_DATE) {
                    ...
                    html_el.innerHTML = current_tei.enrollments?.[0]
                        ?.incidentDate
                        ? dayjs(
                              current_tei.enrollments?.[0]?.incidentDate,
                          ).format("YYYY-MM-DD")
                        : "";
                }
                ...
                if (get_enrollment_html_id === ORGANISATION_UNIT_NAME) {
                    ...
                    html_el.innerHTML =
                        current_tei.enrollments?.[0]?.orgUnitName || "";
                }
            }
        }
        ...
        if (get_data_is === DATA_ELEMENT) {
            if (get_id) {
                ...
                if (get_data_has_legend === "NO") {
                    ...
                    const actuel_event =
                        current_tei?.enrollments[0]?.events?.filter(
                            (ev) => ev.programStage === get_programStage_id,
                        )?.[0];

                    if (
                        actuel_event &&
                        actuel_event?.dataValues?.length > 0 &&
                        get_programStage_id === actuel_event?.programStage &&
                        get_program_id === selectedProgramTrackerFromHTML?.id
                    ) {
                        found_element = actuel_event.dataValues.find(
                            (dv) => dv.dataElement === get_dataElement_id,
                        );
                    }
...
export const injectDataIntoHtml = (
    dataValues,
    { html },
    orgUnits,
    levels,
    ...
    if (selectedOrgUnit) {
        ...
        const html_elements_list = my_container.querySelectorAll(
            '[data-type="AGGREGATE"]',
        );
            ...
            if (data_has_legend === "NO" && html_ID) {
                ...
                if (dx_id && ou_id) {
                    for (let dataValue of dataValues) {
                        ...
                        const el = dataValue.categoryOptionCombo
...
export const generateTreeFromOrgUnits = (
    ouList = [],
    icon = null,
    parentId = null,
    level = 1,
    ...
    let orgUnits = ouList.map((o) => {
        return {
            ...
            children: [],
            parent:
                o.parent !== null && o.parent !== undefined
                    ? o.parent.id
                    : null,
        };
    });
    ...
    let nodes = parentId
        ? orgUnits.filter((o) => o.id === parentId)
        : orgUnits.filter((o) => o.level === level);
            ...
            a.children.forEach((b) => {
                ...
                b.children.forEach((c) => {
                    ...
                    c.children.forEach((d) => {
                        d.children = orgUnits.filter(
                            (org) => org.parent === d.id,
                        );

                        d.children.forEach((e) => {
                            e.children = orgUnits.filter(
                                (org) => org.parent === e.id,
                            );

                            e.children.forEach((f) => {
                                f.children = orgUnits.filter(
                                    (org) => org.parent === f.id,
                                );

                                f.children.forEach((g) => {
                                    g.children = orgUnits.filter(
                                        (org) => org.parent === g.id,
                                    );

                                    g.children.forEach((h) => {
                                        h.children = orgUnits.filter(
                                            (org) => org.parent === h.id,
                                        );

                                        h.children.forEach((i) => {
                                            i.children = orgUnits.filter(
                                                (org) => org.parent === i.id,
                                            );

                                            i.children.forEach((j) => {
                                                j.children = orgUnits.filter(
                                                    (org) =>
                                                        org.parent === j.id,
                                                );
...
export const getOrgUnitIdFromParentString = (
    parent_string,
    selectedOU,
    orgUnits,
    orgUnitLevels,
    ...
    if (parent_string) {
        if (parent_string === CURRENT) {
            ...
            if (orgUnitLevels && orgUnitLevels.length > 0) {
                ...
                const selectedOu_object = orgUnits.find(
                    (ou) => ou.id === selectedOU,
                );
                ...
                if (selectedOu_object.level - parent_string_index > 0) {
                    corresponding_parent_level =
                        selectedOu_object.level - parent_string_index;
                }
                ...
                if (corresponding_parent_level) {
                    const selectedOu_parent_path_list =
                        selectedOu_object.path.split("/");
                    let new_selectOU_parent_path_list = [];
                    for (let path_id of selectedOu_parent_path_list) {
                        const newObject = orgUnits.find(
                            (ou) => ou.id === path_id,
                        );
                        if (newObject) {
                    ...
                    const parent_found = new_selectOU_parent_path_list.find(
                        (ou) => ou.level === corresponding_parent_level,
                    );
                    return parent_found;
                }
            }
...
export const getOrgUnitParentFromHtml = (
    selectedOU,
    orgUnits,
    orgUnitLevels,
) => {
    ...
    const id_html_list =
        document
            .querySelector('[id="my-table-container"]')
            ?.querySelectorAll("[data-type='AGGREGATE']") || [];
        ...
        for (let id_html of id_html_list) {
            ...
            if (id_string) {
                ...
                if (orgUnit_parent_name) {
                    const parent_object = getOrgUnitIdFromParentString(
                        orgUnit_parent_name,
                        selectedOU,
                        orgUnits,
                        orgUnitLevels,
...
export const getAggregateDimensionsList = (report) => {
    ...
    if (report) {
        ...
        if (doc) {
            const aggregateElements = doc.querySelectorAll(
                '[data-type="AGGREGATE"]',
            );
...
export const cleanAggrateDimensionData = (
    report,
    dimensions,
    period,
    periodType,
    ...
    if (report) {
        const aggregateElements = document.querySelectorAll(
            '[data-type="AGGREGATE"]',
        );
        for (let el of aggregateElements) {
            const el_ID = el?.getAttribute("id");
            const legend_id = el_ID?.split("|")?.[2];
            const data_has_organisationUnitGroup = el.getAttribute(
            ...
            for (let d of dimensions) {
                if (el_ID?.includes(d) && legend_id) {
                    ...
                    if (current_legend_parent && current_legend) {
                        if (
                            !data_has_organisationUnitGroup ||
                            data_has_organisationUnitGroup === "NO"
                        ) {
                            // ====> VALEUR MANQUANTE
                        ...
                        if (data_has_organisationUnitGroup === "YES") {
                            ...
                            const ou = getOrgUnitIdFromParentString(
                                OrgUnitString,
                                selectedOU,
                                orgUnits,
                                orgUnitLevels,
                            ...
                            if (ou && ouGId) {
                                if (
                                    ...
                                    defaultMissingValueToApply(
                                        el_ID,
                                        current_legend,
                                    );
                                } else {
                                    defaultNotApplicableValueToApply(
                                        el_ID,
                                        current_legend,
                                    );
                                }
...
const checkIfOuInGroup = (ouGId, ou_id, organisationUnitGroups) => {
    try {
        const organisatinUnitGroup = organisationUnitGroups.find(
            (ouG) => ouG.id === ouGId,
        );

        if (
            organisatinUnitGroup?.organisationUnits
                .map((ou) => ou.id)
                .includes(ou_id)
        ) {
            return true;
...
export const updateAndInjectSchoolNames = (
    report,
    selectedOu,
    organisationUnits = [],
    ...
    if (report) {
        const otherElementsHTML = document.querySelectorAll(
            '[data-type="'.concat(OTHER_ELEMENT).concat('"]'),
        );
        ...
        for (let el of otherElementsHTML) {
            ...
            if (data_is === ORGANISATION_UNIT_NAME) {
                ...
                for (let htmlOu of htmlOUList) {
                    const name_found = getOrgUnitIdFromParentString(
                        htmlOu,
                        selectedOu,
                        organisationUnits,
                        organisationUnitLevels,
...
export const updateAndInjectOtherElementPeriod = (
    report,
    selectedDate,
    selectedPeriodType,
) => {
    if (report) {
        ...
        for (let el of otherElementsHTML) {
            ...
            if (data_is === SELECTED_DATE) {
                el.innerHTML = "";
                el.innerHTML = formatPeriodForAnalytic(
                    selectedDate,
                    selectedPeriodType,
                );
            }
...
export const loadDataStore = async (
    baseUrl,
    key_string,
    setLoading,
    setState,
    ...
    try {
        if (!key_string)
            throw new Error(
                "Please specify the key_string of the datastore to retrieve",
            );

        setLoading && setLoading(true);

        const route = `${baseUrl}${DATA_STORE_ROUTE}/${process.env.REACT_APP_DATA_STORE_NAME}/${key_string}`;
...
export const saveDataToDataStore = async (
    baseUrl,
    key_string,
    payload,
    setLoading,
    ...
    try {
        ...
        if (!payload)
            throw new Error(
                "Please add the payload to save in the datastore !",
            );
        ...
        response = await axios.put(route, payload, { withCredentials: true });
...
export const createDataToDataStore = async (baseUrl, key_string, payload) => {
    try {
        if (!key_string)
            throw new Error(
                "Please specify the key_string of the datastore to retrieve",
...
export const deleteKeyFromDataStore = async (baseUrl, key_string) => {
    try {
        if (!key_string)
            throw new Error(
                "Please specify the key_string of the datastore to retrieve",
        ...
        const route = `${baseUrl}${DATA_STORE_ROUTE}/${process.env.REACT_APP_DATA_STORE_NAME}/${key_string}`;
        await axios.delete(route, { withCredentials: true });
...
export const formatPeriodForAnalytic = (period, periodType) => {
    if (periodType === DAY) return dayjs(period).format("YYYYMMDD");
    if (periodType === YEAR) return dayjs(period).format("YYYY");
    if (periodType === MONTH) return dayjs(period).format("YYYYMM");
};

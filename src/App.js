...
import {
    cleanAggrateDimensionData,
    formatPeriodForAnalytic,
    getAggregateDimensionsList,
    getOrgUnitParentFromHtml,
    injectDataIntoHtml,
    inject_tei_into_html,
    loadDataStore,
    updateAndInjectOtherElementPeriod,
    updateAndInjectSchoolNames,
...
const App = () => {
    ...
    const [isDataStoreReportsCreated, setDataStoreReportsCreated] =
        useState(false);
    ...
    const [loadingDataStoreInitialization, setLoadingDataStoreInitialization] =
        useState(false);
    ...
    const [selectedDataTypeFromHTML, setSelectedDataTypeFromHTML] =
        useState(null);
    ...
    const [selectedProgramTrackerFromHTML, setSelectedProgramTrackerFromHTML] =
        useState(null);
    ...
    const initDataStore = async () => {
        try {
            ...
            const legs = await loadDataStore(
                baseUrl,
            ...
            loadDataStore(
                baseUrl,
                process.env.REACT_APP_SMS_CONFIG_KEY,
            ...
            loadLegendContents(baseUrl, legs);
        } catch (err) {
            setNotif({
                show: true,
    ...
    const initAppUserGroup = async () => {
        try {
            const existedGroup = await axios.get(
                `${baseUrl}${USER_GROUPS_ROUTE}&fields=id&filter=name:eq:${process.env.REACT_APP_USER_GROUP}`,
                { withCredentials: true },
            );
            if (existedGroup.data.userGroups.length === 0) {
                await axios.post(
                    `${baseUrl}${USER_GROUPS_ROUTE}`,
                    {
                        name: process.env.REACT_APP_USER_GROUP,
                    },
                    { withCredentials: true },
                );
    ...
    const loadLegendContents = async (baseUrl, legendList) => {
        try {
    ...
    const loadMe = async (_) => {
        fetch(baseUrl + ME_ROUTE, { credentials: "include" })
    ...
    const loadOrganisationUnitGroups = async (_) => {
        try {
            const request = await fetch(
                baseUrl +
                    ORGANISATION_UNIT_GROUP_ROUTE.concat(
                        "&fields=id,name,displayName,organisationUnits",
                    ),
                { credentials: "include" },
            );
    ...
    const queryTeiList = async (_) => {
        try {
            if (
                ...
                let route = TEIS_ROUTE.concat(
                    "?fields=orgUnit,trackedEntityInstance,attributes,enrollments[*]&ou=",
                )
                ...
                if (searchProperties.length > 0) {
                    searchProperties.forEach((p) => {
                        if (p.value && p.value?.trim() !== "") {
                            route = route.concat(
                                "&attribute="
                                    .concat(p.trackedEntityAttribute?.id)
                                    .concat(":LIKE:" + p.value),
                            );
                        }
                    });
            ...
            <div className="row" style={{ width: "100%", minHeight: "95vh" }}>
                ...
                >
                    <div style={{ position: "relative", height: "100%" }}>
                        ...
                        >
                            {isAuthorised() && (
                                <>
                                    ...
                                    <div
                                        onClick={() => handleDesignPage()}
                                        style={{
                                            display: "flex",
                                            alignItems: "center",
                                        }}
                                        className={"my-menu".concat(
                                            renderPage === PAGE_DESIGN
                                                ? " current"
                                                : "",
                                        )}
                                    >
                                    ...
                                    <div
                                        onClick={() => handleLegendPage()}
                                        style={{
                                            display: "flex",
                                            alignItems: "center",
                                        }}
                                        className={"my-menu".concat(
                                            renderPage === PAGE_LEGEND
                                                ? " current"
                                                : "",
                                        )}
                                    >
                                    ...
                                    <div
                                        onClick={() => handleSmsConfigPage()}
                                        style={{
                                            display: "flex",
                                            alignItems: "center",
                                        }}
                                        className={"my-menu".concat(
                                            renderPage === PAGE_SMS_CONFIG
                                                ? " current"
                                                : "",
                                        )}
                                    >
                        ...
                        >
                            <div
                                style={{ fontSize: "10px", color: "#00000050" }}
                            >
                                HWCA / version: {process.env.REACT_APP_VERSION}
                            </div>
                        </div>
                    </div>
                </div>
    ...
    const RenderListTeiModal = () => (
        ...
        >
                        ...
                        >
                            <Table.Header>
                                {selectedProgramTrackerFromHTML?.programTrackedEntityAttributes?.map(
                                    (att) => (
                                        <Table.Column key={att.id}>
                                            {att?.trackedEntityAttribute?.name}
                                        </Table.Column>
                                    ),
                                )}
            ...
            <Modal.Footer>
                <Button onClick={() => setVisibleListTei(false)}>Close</Button>
            </Modal.Footer>
        </Modal>
    );

...
const LegendPage = ({
    ...
    ``;
    const [visibleSaveLegendPeriodPopup, setVisibleSaveLegendPeriodPopup] =
        useState(false);
    const [visibleAddItemImageList, setVisibleAddItemImageList] =
        useState(false);
    ...
    const [
        ...
        setVisibleDefaultLegendImageMissingData,
    ] = useState(false);
    const [
        visibleDefaultLegendImageNotApplicable,
        setVisibleDefaultLegendImageNotApplicable,
    ] = useState(false);
    const [visibleFinishCreateLegend, setVisibleFinishCreateLegend] =
        useState(false);
    ...
    const [legendDefaultMissingData, setLegendDefaultMissingData] =
        useState(null);
    const [legendDefaultNotApplicable, setLegendDefaultNotApplicable] =
        useState(null);
    const [legendItemColorSelected, setLegendItemColorSelected] =
        useState("#000000");
    const [legendItemImageSelected, setLegendItemImageSelected] =
        useState(null);
    const [legendItemIntervalStart, setLegendItemIntervalStart] =
        useState(null);
    ...
    const [currentLegendIdToDelete, setCurrentLegendIdToDelete] =
        useState(null);

    const [loadingProcess, setLoadingProcess] = useState(false);
    const [loadingCurrentLegendContent, setLoadingCurrentLegendContent] =
        useState(false);
    ...
    const handleSaveAddLegendItem = () => {
        try {
            if (!legendItemName) throw new Error("Item name is required");
            if (!legendItemIntervalEnd)
                throw new Error("Interval End is required");

            if (
                parseFloat(legendItemIntervalEnd) <=
                parseFloat(legendItemIntervalStart)
            )
            ...
            if (editLegendItem && currentLegendItem) {
                ...
                if (
                    legendItemList
                        .map((l) => l.name)
                        .includes(newObject.name) ||
                    legendItemList.map((l) => l.end).includes(newObject.end)
                )
    ...
    const handleDeleteLegend = async (id) => {
        try {
            if (id) {
                ...
                const refreshLegendList = await loadDataStore(
                    baseUrl,
                    process.env.REACT_APP_LEGENDS_KEY,
                    null,
                    null,
                    [],
                );
                ...
                await deleteKeyFromDataStore(baseUrl, `LEGEND_${id}`);
    ...
    const handleEditLegend = async (leg) => {
        try {
            ...
            const legContent = await loadDataStore(
                baseUrl,
                `LEGEND_${leg.id}`,
                null,
                null,
            ...
            if (!legContent) throw new Error(leg.name + " Content not found !");
    ...
    const handleSaveLegend = async () => {
        try {
            ...
            if (editLegend && currentLegend && refreshLegendList) {
                payload = refreshLegendList.map((leg) =>
                    leg.id === currentLegend.id
                        ? { ...leg, name: legendName, updatedAt: dayjs() }
                        : leg,
                );

                const refreshCurrentLegendContent = await loadDataStore(
                    baseUrl,
                    `LEGEND_${currentLegend.id}`,
                    null,
                    null,
                    {},
                );
            ...
            await saveDataToDataStore(
                baseUrl,
                `LEGEND_${payloadLegendContent.id}`,
                payloadLegendContent,
                null,
                null,
                null,
            ...
            const legendList = await loadDataStore(
                process.env.REACT_APP_LEGENDS_KEY,
                setLoadingLegends,
                setLegends,
                [],
            );
            loadLegendContents(baseUrl, legendList);
    ...
    const RenderLegendTable = () => (
        <>
            <div
                className="bg-white p-3 border-bottom"
            ...
            <div className="p-3">
                <div className="bg-white p-2 rounded my-shadow">
                    <div className="text-right">
                        <Button primary onClick={handleNewLegendBtn}>
                            + New Legend
                        </Button>
                    </div>
    ...
    const getPeriodSetNameFromPeriod = (start, end) => {
        let name = "";
        if (start) name = dayjs(start).format("YYYY-MM-DD");

        if (end) name = `${name}_${dayjs(end).format("YYYY-MM-DD")}`;
        ...
        if (!end) {
            const periods =
                periodList
                    ...
                    ?.filter((p) =>
                        currentLegendPeriod
                            ? getPeriodSetNameFromPeriod(p.start, p.end) !==
                                  currentLegendPeriod.name && !p.end
                            : !p.end,
                    ) || [];
            if (periods.length > 0)
                throw new Error(
                    "There are already a period set with no end date created. May be you can close the period set with no end date  before add this !",
                );
            return true;
        } else {
            return true;
        ...
        if (!end) {
            const periods = periodList
                ...
                .filter((p) =>
                    currentLegendPeriod
                        ? getPeriodSetNameFromPeriod(p.start, p.end) !==
                          currentLegendPeriod.name
                        : true,
                )
                .reduce((prev, current) => {
                    const period = current;
                    if (
                        dayjs(period.start).isSame(dayjs(start)) ||
                        dayjs(period.start).isBefore(dayjs(start))
                    ) {
                        prev.push(current);
                    }
                    return prev;
            ...
            if (periods.length > 0)
                throw new Error(
                    "There are already existing date range which cover your selected dates !",
                );
            ...
            const periods = periodList
                ...
                .reduce((prev, current) => {
                    ...
                    if (
                        (dayjs(period.start).isSame(dayjs(start)) ||
                            dayjs(period.start).isBefore(dayjs(start))) &&
                        ((period.end && dayjs(period.end).isSame(dayjs(end))) ||
                            (period.end &&
                                dayjs(period.end).isAfter(dayjs(end))))
                    ) {
                        prev.push(current);
                    }
                    return prev;
    ...
    const handleSaveLegendPeriod = async () => {
        try {
            await validateDateIfThereIsNoEndDate(
                legendPeriods.map((l) => l.name),
                startDateForLegendSet,
                endDateForLegendSet,
            );

            if (
                endDateForLegendSet &&
                dayjs(startDateForLegendSet).isAfter(endDateForLegendSet)
            )
                throw new Error(
                    "The End Date must be gratter than Start Date !",
                );
            ...
            let payload = {
                name: getPeriodSetNameFromPeriod(
                    startDateForLegendSet,
                    endDateForLegendSet,
                ),
                min: null,
            ...
            setLegendPeriods([payload, ...legendPeriods]);
            setStartDateForLegendSet(null);
            setEndDateForLegendSet(null);
            setVisibleSaveLegendPeriodPopup(false);
            setIsLegendCloned(false);
            ...
            return setNotif({
                show: true,
                message: isLegendCloned ? "Legend Cloned !" : "Legend added !",
                type: NOTIFICATON_SUCCESS,
            });
        } catch (err) {
            setVisibleSaveLegendPeriodPopup(false);
    ...
    const RenderAddLegendPeriodModal = () =>
        visibleSaveLegendPeriodPopup && (
            <Modal onHidden={handleCancelSaveLegendPeriodModal} small dense>
                <ModalTitle>
                    {" "}
                    <div style={{ fontWeight: "bold", fontSize: "15px" }}>
                        {" "}
                        {currentLegendPeriod && !isLegendCloned
                            ? "Update period "
                            : "New period "}{" "}
                    </div>
                </ModalTitle>
                        ...
                        <div>
                            ...
                            <div style={{ marginTop: "5px" }}>
                                <DatePicker
                                    style={{ width: "100%" }}
                                    picker="date"
                                    value={startDateForLegendSet}
                                    onChange={(value) =>
                                        setStartDateForLegendSet(value)
                                    }
                                />
                            </div>
                        </div>
                        <div style={{ marginTop: "10px" }}>
                            ...
                            <div style={{ marginTop: "5px" }}>
                                <DatePicker
                                    style={{ width: "100%" }}
                                    picker="date"
                                    value={endDateForLegendSet}
                                    onChange={(value) =>
                                        setEndDateForLegendSet(value)
                                    }
                                />
                            </div>
                        </div>
                ...
                <ModalActions>
                    <ButtonStrip end>
                        <Button
                            ...
                            onClick={() =>
                                currentLegendPeriod && !isLegendCloned
                                    ? handleCancelLegendPeriodModification()
                                    : handleCancelSaveLegendPeriodModal()
                            }
                            destructive
                        >
                            {" "}
                        ...
                        <Button
                            ...
                            onClick={() =>
                                currentLegendPeriod && !isLegendCloned
                                    ? handleSaveLegendPeriodModification()
                                    : handleSaveLegendPeriod()
                            }
                        >
                            Save Period
                        </Button>
    ...
    const RenderFinishLegendCreationModal = () =>
        visibleFinishCreateLegend && (
                    ...
                    <div>
                        <Field label="Legend name">
                            <Input
                                ...
                                value={legendName}
                                onChange={({ value }) => setLegendName(value)}
                            />
                        </Field>
                    </div>
                </ModalContent>
                <ModalActions>
                    <ButtonStrip end>
                        ...
                        <Button
                            ...
                            disable={loadingProcess || !legendName}
                            primary
        ...
        try {
            ...
            if (current_file) {
                if (parseInt(current_file.size) / 1024 > 100)
                    throw new Error(
                        "The file is too big. The file must not be gratter than 100Kb",
                    );
    ...
    const RenderAddItemModal = () =>
        visibleAddLegendItem ? (
            <Modal onHidden={handleCancelAddItemLegend}>
                <ModalTitle> New Legend item </ModalTitle>
                <ModalContent>
                    {errorMessageLegendItem && (
                        <NoticeBox title="error" small dense error>
                            {" "}
                            {errorMessageLegendItem}{" "}
                        </NoticeBox>
                    )}
                                ...
                                <div className="col-md">
                                    <Field label="Start value">
                                        <Input
                                            ...
                                            type="number"
                                            value={legendItemIntervalStart}
                                            onChange={({ value }) =>
                                                setLegendItemIntervalStart(
                                                    value,
                                                )
                                            }
                                        />
                                ...
                                <div className="col-md">
                                    <Field label="End value">
                                        <Input
                                            ...
                                            type="number"
                                            value={legendItemIntervalEnd}
                                            onChange={({ value }) =>
                                                setLegendItemIntervalEnd(value)
                                            }
                                        />
                                    </Field>
                                </div>
                            ...
                            <div className="mt-3">
                                <Field label="Item name">
                                    <Input
                                        ...
                                        value={legendItemName}
                                        onChange={({ value }) =>
                                            setLegendItemName(value)
                                        }
                                    />
                                </Field>
                            </div>
                ...
                <ModalActions>
                    <ButtonStrip end>
                        <Button
                            small
                            onClick={handleCancelAddItemLegend}
                            destructive
                        >
                            {" "}
                            Cancel{" "}
                        </Button>
                        <Button small onClick={handleSaveAddLegendItem} primary>
                            {" "}
                            Save item
                        </Button>
                    </ButtonStrip>
    ...
    const RenderLegendItemList = () => (
        <div className="bg-white border rounded p-3 my-shadow">
            <div className="d-flex justify-content-between">
                ...
                >
                    ...
                    {currentLegendPeriod && (
                        <>
                            {" "}
                            ({" "}
                            {getPeriodSetNameFromStringAsPeriodName(
                                currentLegendPeriod.name,
                            )}{" "}
                            ){" "}
                        </>
                ...
                <Button
                    className="bg-light"
                    onClick={handleNewLegendItemBtn}
                    small
                >
                    + New Item
                </Button>
            ...
            <div className="mt-1">
                <Table>
                    ...
                    <TableBody>
                        {legendItemList.length > 0 ? (
                            legendItemList.map((leg) => (
                                <TableRow key={leg.id}>
                                    <TableCell dense>{leg.name}</TableCell>
                                    <TableCell dense>
                                        {""
                                            .concat(leg.start)
                                            .concat(" <===> ")
            ...
            <div className="row">
                <div className="col-md">
                    <Field label="Default name if data missing">
                        <Input
                            ...
                            value={legendDefaultMissingData}
                            onChange={({ value }) =>
                                setLegendDefaultMissingData(value)
                            }
                        />
                    </Field>
                </div>
                <div className="col-md">
                    <Field label="Default name if data is not applicable">
                        <Input
                            placeholder="Not applicable legend"
                            value={legendDefaultNotApplicable}
                            onChange={({ value }) =>
                                setLegendDefaultNotApplicable(value)
                            }
                        />
                    </Field>
                </div>
                    ...
                    <div>
                        ...
                        <div>
                            <div className="d-flex align-items-center mt-1">
                                {legendDefaultMissingData && (
                                    <div className="mr-2 mt-2">
                                        <img
                                            src={legendDefaultMissingData}
                                            style={{
                                                height: "30px",
                                                width: "30px",
                                            }}
                        ...
                        <div>
                            <div className="d-flex align-items-center mt-1">
                                {legendDefaultNotApplicable && (
                                    <div className="mr-2 mt-2">
                                        <img
                                            src={legendDefaultNotApplicable}
                                            style={{
                                                height: "30px",
                                                width: "30px",
                                            }}
                    ...
                    <div>
                        ...
                        <div className="d-flex mt-1">
                            ...
                            <div>
                                <SketchPicker
                                    color={
                                        legendDefaultMissingData || "#000000"
                                    }
                                    onChange={(color) =>
                                        setLegendDefaultMissingData(color.hex)
                                    }
                                />
                            </div>
                        </div>
                    ...
                    <div>
                        ...
                        <div className="d-flex mt-1">
                            ...
                            <div>
                                <SketchPicker
                                    color={
                                        legendDefaultNotApplicable || "#000000"
                                    }
                                    onChange={(color) =>
                                        setLegendDefaultNotApplicable(color.hex)
                                    }
                                />
                            </div>
                        </div>
    ...
    const handleSaveLegendPeriodModification = async () => {
        try {
            ...
            if (!currentLegendPeriod) throw new Error("Please select period");

            if (!min) throw new Error("The min value is required");

            if (!max) throw new Error("The max value is required");

            if (!intervalSpace) throw new Error("The interval is required");
            ...
            if (!legendDefaultMissingData)
                throw new Error(
                    "The Default value when data is missing is required",
                );

            if (!legendDefaultNotApplicable)
                throw new Error(
                    "The Default value when data is not application is required",
                );
            ...
            setLegendPeriods(
                legendPeriods.map((leg) =>
                    leg.name === currentLegendPeriod.name ? payload : leg,
                ),
            );
            setNotif({
                show: true,
                message: `Period ${currentLegendPeriod.name} has been updated`,
                type: NOTIFICATON_SUCCESS,
            });
    ...
    const handleGenerateLegendOption = () => {
        ...
        if (!min) return setGenerateErrorMessage("The min value is required");

        if (!max) return setGenerateErrorMessage("The max value is required");

        if (!intervalSpace)
            return setGenerateErrorMessage("The interval is required");

        if (parseFloat(max) <= parseFloat(min))
            return setGenerateErrorMessage(
                "The Max value must be gratter than min value",
            );

        const pas =
            (parseFloat(max) - parseFloat(min)) / parseFloat(intervalSpace);
        ...
        for (
            ...
            i <= parseFloat(max);
            i = parseFloat(i) + parseFloat(pas)
        ) {
            // if (Math.round(i) < Math.round(max)) {
            if (
                parseFloat(parseFloat(i).toFixed(4)) <
                parseFloat(parseFloat(max).toFixed(6))
            ) {
                const start = i;
        ...
        <div className="border rounded p-3 my-shadow bg-white">
            ...
            <div className="row mt-2">
                ...
                <div className="col-md-12 mt-3">
                    <Button onClick={handleGenerateLegendOption} small primary>
                        Generate legend options
                    </Button>
                </div>
            ...
            <div className="mt-3">
                <Field label="Legend Type">
                    <SingleSelect
                        ...
                        onChange={handleChangeTypeLegend}
                        selected={legendDefaultType}
                    >
            ...
            <div className="mt-4 d-flex">
                <div>
                    <Button
                        destructive
                        onClick={handleCancelLegendPeriodModification}
                    >
                        Cancel
                    </Button>
                </div>
    ...
    const handleEditLegendPeriod = (value) => {
        ...
        setStartDateForLegendSet(
            getPeriodSetNameFromStringAsPeriodObject(value.name)?.start &&
                dayjs(
                    getPeriodSetNameFromStringAsPeriodObject(value.name)?.start,
                ),
        );
        setEndDateForLegendSet(
            getPeriodSetNameFromStringAsPeriodObject(value.name)?.end &&
                dayjs(
                    getPeriodSetNameFromStringAsPeriodObject(value.name)?.end,
                ),
        );
    ...
    const handleDeletelegendPeriod = (name) => {
        try {
            if (name) {
                ...
                setLegendPeriods(
                    legendPeriods.filter((el) => el.name !== name),
                );
                ...
                <div className="mt-2">
                    ...
                    {legendPeriods.length > 0 && (
                        <TableAntd
                            ...
                            style={{ width: "100%" }}
                            pagination={false}
                            dataSource={legendPeriods
                                .sort((a, b) => {
                                    const aObject =
                                        getPeriodSetNameFromStringAsPeriodObject(
                                            a.name,
                                        );
                                    const bObject =
                                        getPeriodSetNameFromStringAsPeriodObject(
                                    ...
                                    return dayjs(aObject.start).isBefore(
                                        bObject.start,
                                    )
                                        ? 1
                                        : -1;
    ...
    const RenderLegendPeriodElementContent = () => (
        <div className="col-md-9">
            <div className="row">
                <div className="col-md-5">{RenderNewLegendGroupForm()}</div>
                <div className="col-md-7">{RenderLegendItemList()}</div>
            </div>
        </div>
    );
    ...
    const RenderNewLegendForm = () => (
        <>
            ...
            >
                <div style={{ fontWeight: "bold", fontSize: "15px" }}>
                    {" "}
                    {currentLegend && editLegend
                        ? `Update legend : ( ${currentLegend?.name} )`
                        : "New Legend"}{" "}
                </div>
                <div className="d-flex">
                    <Button destructive onClick={handleCancelSaveLegend}>
                        Cancel
                    </Button>
                    <Button
                        icon={
            ...
            <div className="row p-2">
                {editLegend && currentLegend && loadingCurrentLegendContent && (
                ...
                {editLegend &&
                    currentLegend &&
                    !loadingCurrentLegendContent &&
                    !currentLegendContent && (
                        <div className="col-md-12">
                            <NoticeBox
                                warning
                            >{`${currentLegend?.name} Content not found !`}</NoticeBox>
                        </div>
                    )}
                {RenderPeriodLegendList()}
                {currentLegendPeriod &&
                    !isLegendCloned &&
                    RenderLegendPeriodElementContent()}
            </div>
        </>
    );

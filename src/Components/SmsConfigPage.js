...
const SmsConfigPage = ({
    ...
    const [visibleAddAttributeModal, setVisibleAddAttributeModal] =
        useState(false);
    ...
    const handleSaveReport = () => {
        if (selectedProgram && selectedAttribute && currentHtmlTagSelected) {
            const ID_string = "SMS-"
                .concat(selectedProgram.id)
                .concat("-")
                .concat(selectedAttribute.id);
            const NAME_String = "( "
                .concat(selectedProgram.name)
                .concat(" - ")
                .concat(selectedAttribute.name)
                .concat(" )");
        ...
        visibleAddAttributeModal ? (
            <Modal small onClose={handleCloseSaveModal}>
                ...
                <ModalContent>
                    ...
                    {selectedProgram?.programTrackedEntityAttributes?.length >
                        0 && (
                        <div className="mt-2">
                            <Field label="Select Attribute ">
                                <SingleSelect
                                    placeholder="program attribute "
    ...
    const handleSaveTemplate = async () => {
        try {
            ...
            const html_code = window
                .$("#summernote-sms-template")
                ?.summernote("code");
            ...
            const payload =
                smsConfigs && smsConfigs.length > 0
                    ? [
                          {
                              id: uuid(),
                              name: templateName,
            ...
            setNotif({
                message: "SMS Template is created",
                show: true,
    ...
    const SaveModal = () =>
        visibleSaveModal ? (
            <Modal small onClose={handleCloseAttributeModal}>
                <ModalTitle>SMS Template</ModalTitle>
                <ModalContent>
                    <Field label="SMS Template name">
                        <Input
                            onChange={({ value }) =>
                                setTemplateName("".concat(value))
                            }
                            value={templateName}
                ...
                <ModalActions>
                    <ButtonStrip end>
                        ...
                        <Button
                            ...
                            primary
                            disabled={loadingProcess ? true : false}

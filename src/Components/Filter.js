...
import {
    ...
    ORGANISATION_UNIT_LEVELS_ROUTE,
    ME_ROUTE,
    PROGRAMS_ROUTE,
} from "../api.routes";
...
const Filter = ({
    ...
    const loadOrgUnitLevels = async (_) => {
        try {
            ...
            const me_request = await fetch(
                baseUrl + ME_ROUTE.concat(",organisationUnits"),
                { credentials: "include" },
            );
            const me_response = await me_request.json();

            if (me_response.status === "ERROR") throw new me_response();

            const userOrganisationUnitId =
                me_response.organisationUnits.length !== 0 &&
                me_response.organisationUnits[0].id;
            if (userOrganisationUnitId) {
                const request = await fetch(
                    baseUrl + ORGANISATION_UNIT_LEVELS_ROUTE,
                    { credentials: "include" },
                );
                const response = await request.json();
                if (response.status === "ERROR") throw response;

                const levels = response.organisationUnitLevels.map(
                    (lvl) => lvl.level,
                );
                const orgLvl = response.organisationUnitLevels.sort(
                    (a, b) => a.level - b.level,
                );
            ...
            const request = await fetch(
                baseUrl +
                    PROGRAMS_ROUTE.concat(
                        "?fields=id,name,programTrackedEntityAttributes[id,name, program[id,name], trackedEntityAttribute[id,name,display]]",
                    )
                        .concat(
    ...
    const loadOrganisationUnits = async (orgUnitLevels) => {
        try {
            const requestOU = await fetch(
                baseUrl +
                    ORGANISATION_UNITS_ROUTE.concat(
                        "&fields=id,name,level,parent, path, displayName",
                    ),
                { credentials: "include" },
            );
            const responseOU = await requestOU.json();
            if (responseOU.status === "ERROR") throw responseOU;
        ...
        if (currentReport) {
            ...
            if (doc) {
                ...
                for (let el of trackerElement) {
                    ...
                    if (
                        data_is === DATA_ELEMENT &&
                        !data_element_list.includes(id_dataElement)
                    ) {
                ...
                setProgramTrackersFromHTML(
                    trackerprog.map((el_id) =>
                        programs.find((prog_id) => prog_id.id === el_id),
                    ),
                );
                trackerprog.length > 0 &&
                    setSelectedProgramTrackerFromHTML(
                        programs.find(
                            (prog_id) => prog_id.id === trackerprog[0],
                        ),
                    );
                data_element_list.length > 0 &&
                    setDataElementsFromHTML(data_element_list);
            }
        }
    };
    ...
    const handleSelectReport = async ({ selected }) => {
        try {
            ...
            const currentReportContent = await loadDataStore(
                baseUrl,
                `REPORT_${currentReport?.id}`,
                null,
                null,
                {},
            );
            ...
            if (currentReport && currentReportContent) {
                ...
                const doc = parser.parseFromString(
                    currentReportContent.html,
                    "text/html",
                );
                const aggregateElement = doc.querySelectorAll(
                    "[data-type='AGGREGATE']",
                );
                const trackerElement = doc.querySelectorAll(
                    "[data-type='TRACKER']",
    ...
    const OrganisationUnitModal = () =>
        visibleOrgUnit ? (
            <Modal onClose={() => handleCloseOUModal()} large>
                <ModalTitle>Organisation Units</ModalTitle>
                <ModalContent>
                    <div>
                        ...
                        {currentOrgUnits.length > 0 && (
                            ...
                            >
                                <div className="mr-2"> Org Unit:</div>{" "}
                                <div
                                    style={{
                                        fontWeight: "bold",
                ...
                <ModalActions>
                    <ButtonStrip end>
                        ...
                        <Button
                            onClick={() => setVisibleOrgUnit(false)}
                            primary
                        >
                            Ok
                        </Button>
    ...
    const handleSelectedProgramTrackerFromHTML = (selected) => {
        setSelectedProgramTrackerFromHTML(
            programTrackersFromHTML.find((p) => p.id === selected),
        );
    ...
    const handleSelectSearchProperties = (att) => {
        if (!searchProperties.map((s) => s.id).includes(att.id)) {
            ...
            setSearchProperties(
                searchProperties.filter((s) => s.id !== att.id),
            );
        }
    };
    ...
    const TrackerDataTypeContent = () => (
        <>
            {programTrackersFromHTML.length > 0 && (
                <div className="mt-2">
                    <Field label="Program">
                        <SingleSelect
                            ...
                            onChange={({ selected }) =>
                                handleSelectedProgramTrackerFromHTML(selected)
                            }
                        >
            ...
            {selectedProgramTrackerFromHTML && (
                <div className="mt-4">
                    <div
                        ...
                        onClick={() => setVisibleOrgUnit(true)}
                    >
                        {currentOrgUnits.length > 0
                            ? currentOrgUnits[0].name
            ...
            {searchByAttribute && selectedProgramTrackerFromHTML && (
                <div className="mt-3">
                    <div className="my-shadow bg-white border rounded p-1">
                        ...
                        <div
                            className="collapse"
                            id="searchByAttributeCollapse"
                        >
                            {selectedProgramTrackerFromHTML?.programTrackedEntityAttributes?.map(
                                (att) => (
                                    <Checkbox
                                        ...
                                        key={att.id}
                                        onChange={({ checked }) =>
                                            handleSelectSearchProperties(att)
                                        }
                                    />
                                ),
                            )}
            ...
            {!searchByAttribute && selectedProgramTrackerFromHTML && (
                <div className="mt-3">
                    <Button
                        ...
                        loading={
                            loadingQueryTeiList || loadingOrganisationUnits
                        }
                        onClick={queryTeiList}
                    >
                        Update report
                    </Button>
                </div>
    ...
    const AggregateDataTypeContent = () => (
        <>
            <div className="mt-3">
                <PeriodFieldType
                    setState={setSelectedPeriodType}
            ...
            loadingLegendContents ? (
                ...
                >
                    ...
                    <span style={{ marginLeft: "5px" }}>
                        Loading...{" "}
                        <span style={{ marginLeft: "5px", color: "#00000090" }}>
                            {" "}
                            {calculatePercentage(
                                legendContents.length || 0,
                                legends.length || 0,
                            )}{" "}
                            %
                        </span>
                ...
                <div>
                    <div className="mt-2">
                        <Field label="Which report ?">
                            ...
                            >
                                ...
                                {isDataStoreReportsCreated &&
                                    reports?.length === 0 && (
                                        <SingleSelectOption label="No Report" />
                                    )}
                            </SingleSelect>
                        </Field>
                    </div>
                    ...
                    {dataTypesFromHTML.length > 0 && (
                        <div className="mt-2">
                            <div className="my-2">
                                <NoticeBox title="Report">
                                    As your report contains tracker and
                                    aggregate data , you must generate your
                                    report by selecting data type one by one
                                </NoticeBox>
                            </div>

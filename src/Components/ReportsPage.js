...
const ReportsPage = ({
    ...
    const printReportAsPDF = () => {
        let reportDocument = document.querySelector(
        ...
        reportDocument.querySelectorAll("canvas").forEach((cv) => {
            ...
            current_canvas_parent.innerHTML =
                "<img src='" + current_canvas_url + "' />";
        });
    ...
    const handleSendSMS = async () => {
        try {
            ...
            if (!selectedTEI) throw new Error("Please Select tei");
            ...
            if (!HTML_TEMPLATE) throw new Error("No html template found ");

            let HTML_TEMPLATE_DOM = parser.parseFromString(
                HTML_TEMPLATE,
                "text/html",
            );

            const all_elements =
                HTML_TEMPLATE_DOM.querySelectorAll("[id*='SMS'");

            for (let at of all_elements) {
                const id = at.getAttribute("id")?.split("-");
                const programID = id[1];
                const attributeID = id[2];
                const getAttributeTEI = selectedTEI.attributes?.find(
                    (tei) => tei.attribute === attributeID,
                );

                if (
                    getAttributeTEI &&
                    selectedTEI.enrollments?.[0]?.program === programID
                ) {
                    const founded_el = HTML_TEMPLATE_DOM.querySelector(
                        "[id='" + at.getAttribute("id") + "']",
                    );
                    if (founded_el) {
            ...
            if (!TEMPLATE_PLAIN_TEXT) throw new Error("No SMS Template");
            ...
            const payload = {
                ...
                recipients: [
                    selectedTEI.attributes.find(
                        (at) => at.attribute === "YnHuqCmuydE",
                    )?.value,
                ],
            };

            const request = await fetch(baseUrl + SMS_ROUTE_API, {
                method: "post",
        ...
        visibleSendSMSModal ? (
            <Modal small onClose={() => setVisibleSendSMSModal(false)}>
                    ...
                    {smsConfigs?.length > 0 ? (
                        <Field label="SMS Template ">
                            <SingleSelect
                                ...
                                onChange={({ selected }) =>
                                    setSelectedTemplate(
                                        smsConfigs?.find(
                                            (conf) => conf.id === selected,
                                        ),
                                    )
                                }
                            >
                                {smsConfigs.map((template) => (
                                    <SingleSelectOption
                                        key={template.id}
                                        value={template.id}
                                        label={template.name}
                        ...
                        <div className="my-3">
                            <NoticeBox error title="SMS Template configuration">
                                No SMS Template configuration founded ! please
                                make sur you have create one SMS Template
                            </NoticeBox>
                        </div>
                    )}
                </ModalContent>
                <ModalActions>
                    <ButtonStrip end>
                        <Button
                            onClick={() => setVisibleSendSMSModal(false)}
                            secondary
                        >
                            close
                        </Button>
                        <Button
                            ...
                            primary
                            disabled={
                                loadingSendSMS || !selectedTemplate
                                    ? true
                                    : false
                            }
                            loading={loadingSendSMS ? true : false}
                        >
                            Send SMS
                        </Button>
                    </ButtonStrip>
    ...
    useEffect(() => {
        if (selectedReport) {
            const aggregateElement = document.body.querySelectorAll(
                "[data-type=" + AGGREGATE.value + "]",
            );
            const trackerElements = document.body.querySelectorAll(
                "[data-type=" + TRACKER.value + "]",
            );
            const otherElements = document.body.querySelectorAll(
                "[data-type=" + OTHER_ELEMENT + "]",
            );

            if (trackerElements && trackerElements.length > 0) {
                trackerElements.forEach((el) => {
                    const dataAttributeValueType = el.getAttribute(
                        "data-attribute-value-type",
                    );
                    if (
                        dataAttributeValueType &&
                        dataAttributeValueType === IMAGE
                    ) {
    ...
    return (
        <div>
            <div
                className="bg-white py-2 d-flex align-items-center justify-content-center my-shadow"
                style={{ position: "sticky", top: "0px", zIndex: 100 }}
            >
                <div className="mr-2">
                    <TbReport style={{ fontSize: "50px", color: "#06695C" }} />
                </div>
                <div style={{ fontSize: "24px" }} className="font-weight-bold">
                    Report Builder
                </div>
            </div>
            <div style={{ padding: "10px" }}>
                {selectedReport && searchProperties && searchByAttribute && (
                    <div className="d-flex align-items-center justify-content-center mt-2">
                        ...
                        {searchProperties.map((p, index) => (
                            <Input
                                className="ml-2"
                                placeholder={p.trackedEntityAttribute?.name}
                                value={p.value}
                                onChange={({ value }) =>
                                    handleSearchInput(value, index)
                                }
                            />
                        ))}
                        <Button
                            className="ml-2"
                            loading={loadingTeiProcess}
                            disabled={currentOrgUnits.length > 0 ? false : true}
                ...
                {selectedReport && (
                    <div className="mt-2 d-flex justify-content-center align-items-center">
                        <Button primary onClick={printReportAsPDF}>
                            Print report
                        </Button>
                        {selectedTEI && (
                            <Button
                                ...
                                onClick={() => setVisibleSendSMSModal(true)}
                            >
                                Send SMS
                            </Button>
                        )}

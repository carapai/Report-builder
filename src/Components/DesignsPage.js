...
const DesignsPage = ({
    ...
    const [currentDimensionSelected, setCurrentDimensionSelected] =
        useState(null);
    ...
    const [
        ...
        setSelectedConfigDimensionsAggregate,
    ] = useState([]);
    const [selectedTypeLegendToDisplay, setSelectedTypeLegendToDisplay] =
        useState(null);
    const [selectedOuLevelToDisplay, setSelectedOuLevelToDisplay] =
        useState(null);
    const [selectedOuGroupToDisplay, setSelectedOuGroupToDisplay] =
        useState(null);
    const [
        selectedDimensionContentToDisplay,
        setSelectedDimensionContentToDisplay,
    ] = useState(AGGREGATE.value);
    const [selectedRadioTypeLegend, setSelectedRadioTypeLegend] =
        useState("VALUE");
    const [
        selectedTrackerConfigDimensionsPrograms,
        setSelectedTrackerConfigDimensionsPrograms,
    ] = useState([]);
    const [selectedProgram, setSelectedProgram] = useState(null);
    const [selectedProgramToDisplay, setSelectedProgramToDisplay] =
        useState(null);
    const [selectedRadioDataToDisplay, setSelectedRadioDataToDisplay] =
        useState(ATTRIBUTE);
    const [selectedProgramStageToDisplay, setSelectedProgramStageToDisplay] =
        useState(null);
    const [
        selectedDataElementDimensionToDisplay,
        setSelectedDataElementDimensionToDisplay,
    ] = useState(null);
    const [selectedAttributeToDisplay, setSelectedAttributeToDisplay] =
        useState(null);
    const [
        selectedEnrollmentDataToDisplay,
        setSelectedEnrollmentDataToDisplay,
    ] = useState(null);
    const [
        selectedAttributeValueTypeToDisplay,
        setSelectedAttributeValueTypeToDisplay,
    ] = useState(null);
    const [_, setSelectedAttributeIsSearchable] = useState(false);
    const [
        selectedOtherOrganisationUnitElements,
    ...
    const [visibleSelectDXTrackerModal, setVisibleSelectDXTrackerModal] =
        useState(false);
    const [visibleSelectDXAggregateModal, setVisibleSelectDXAggregateModal] =
        useState(false);

    const [
        visibleOtherOrganisationUnitElementModal,
        setVisibleOtherOrganisationUnitElementModal,
    ] = useState(false);
    ...
    const handleSaveReport = async () => {
        try {
            ...
            const refreshReportList = await loadDataStore(
                baseUrl,
                process.env.REACT_APP_REPORTS_KEY,
                null,
                null,
                [],
            );
            ...
            if (editReport && currentRepport && currentRepportContent) {
                ...
                const refreshCurrentReportContent = await loadDataStore(
                    baseUrl,
                    `REPORT_${currentRepport.id}`,
                    null,
                    null,
                    {},
                );
                ...
                if (refreshReportList.map((r) => r.name).includes(reportName))
                    throw new Error(
                        "This report is already created ( May be you can change the name ) ",
                    );
            ...
            await saveDataToDataStore(
                baseUrl,
                `REPORT_${payloadReportContent.id}`,
                payloadReportContent,
                null,
                null,
                null,
    ...
    const handleInjectTrackerIds = () => {
        if (selectedRadioDataToDisplay && selectedProgramToDisplay) {
            ...
            if (
                ...
                if (selectedRadioTypeLegend === "LEGEND") {
                    ...
                    name_string = ""
                        ...
                        .concat(
                            (legendToUse &&
                                legends.find((l) => l.id === legendToUse)
                                    ?.name) ||
                                "",
                        )
                        .concat("|")
                ...
                window
                    .$(currentHtmlTagSelected)
                    .attr("data-has-legend", has_legend);
            ...
            if (
                ...
                window
                    .$(currentHtmlTagSelected)
                    .attr("data-is", "DATA_ELEMENT");
                window.$(currentHtmlTagSelected).html("");
                window.$(currentHtmlTagSelected).html(name_string);
            }

            if (
                selectedProgramToDisplay &&
                ...
                id_string = ""
                    .concat(selectedProgramToDisplay.id)
                    .concat("|")
                    .concat(selectedEnrollmentDataToDisplay);
                name_string = ""
                    .concat(selectedProgramToDisplay.name)
                    .concat("|")
                    .concat(selectedEnrollmentDataToDisplay);
    ...
    const handleInjectAggregateIds = () => {
        if (currentDimensionSelected) {
            ...
            let name_string = "";
            let has_legend = "NO";
            let has_organisation_unit_group = selectedOuGroupToDisplay
                ? "YES"
                : "NO";
            ...
            if (selectedOuLevelToDisplay && selectedRadioTypeLegend) {
                if (selectedRadioTypeLegend === "VALUE") {
                    id_string = ""
                        ...
                        .concat(
                            selectedOuGroupToDisplay
                                ? "|".concat(selectedOuGroupToDisplay)
                                : "",
                        );
                    ...
                    name_string = ""
                        ...
                        .concat("|")
                        .concat(
                            selectedOuLevelToDisplay === "CURRENT"
                                ? "CURRENT"
                                : organisationUnitLevels.find(
                                      (lvl) =>
                                          lvl.id === selectedOuLevelToDisplay,
                                  )?.name || selectedOuLevelToDisplay,
                        )
                        .concat(
                            selectedOuGroupToDisplay
                                ? "|".concat(
    ...
    const getParentObjectList = () => {
        ...
        if (organisationUnitLevels && organisationUnitLevels.length > 0) {
            for (let i = 1; i <= organisationUnitLevels.length - 1; i++) {
                newList.push({
                    name: "Parent ".concat(i),
                    value: "PARENT_".concat(i),
                });
            }
        }
    ...
    const getParentListFields = () =>
        getParentObjectList().map((ou) => (
            <SingleSelectOption label={ou.name} value={ou.value} />
        ));

    const RenderOrganisationUnitName = () => {};

    const RenderDisplayWay = selectedProgramToDisplay && (
        <div className="mt-2">
            <Field label="Display Way">
                ...
                <Radio
                    ...
                    checked={
                        selectedRadioTypeLegend === "LEGEND" ? true : false
                    }
                />
            </Field>
        </div>
    ...
    const RenderDataToUsed = selectedProgramToDisplay && (
        <div className="mt-2">
            <Field label="Data to used">
                <Radio
                    ...
                    checked={
                        selectedRadioDataToDisplay === ATTRIBUTE ? true : false
                    }
                />
                ...
                <Radio
                    ...
                    checked={
                        selectedRadioDataToDisplay === ENROLLMENT ? true : false
                    }
                />
                    ...
                    <div className="mt-2">
                        ...
                        <Field label="Choose legend to apply ">
                            <SingleSelect
                                ...
                                selected={legendToUse}
                                onChange={({ selected }) =>
                                    handleChooseLegendToDisplay(selected)
                                }
                            >
                                {legends.length > 0 &&
                                    legends.map((leg) => (
            ...
            {selectedRadioTypeLegend === "LEGEND" && legendToUse && (
                <div className="mt-2">
                    <Field label="Choose legend to display">
                        <SingleSelect
                            placeholder="Choose legend to display"
                            selected={selectedTypeLegendToDisplay}
                            onChange={({ selected }) =>
                                setSelectedTypeLegendToDisplay(selected)
                            }
                        >
    ...
    const RenderAttributeContent = selectedProgramToDisplay && (
        <div className="mt-2">
            {selectedProgramToDisplay.programTrackedEntityAttributes?.length >
                0 && (
                        ...
                        onChange={({ selected }) => {
                            const attr_object =
                                selectedProgramToDisplay.programTrackedEntityAttributes
                                    .map(
                                        (pgAtt) => pgAtt.trackedEntityAttribute,
                                    )
                                    .find((att) => att.id === selected);
                            if (attr_object) {
                                setSelectedAttributeToDisplay(attr_object);
                                setSelectedAttributeValueTypeToDisplay(
                                    attr_object.valueType,
                                );
                            }
                        }}
                    >
                        {selectedProgramToDisplay.programTrackedEntityAttributes.map(
                            (p) => (
                                <SingleSelectOption
                                    key={p.id}
                                    value={p.trackedEntityAttribute?.id}
                                    label={p.trackedEntityAttribute?.name}
                                />
                            ),
                        )}
                    </SingleSelect>
    ...
    const handleSelectedProgramStage = ({ selected }) => {
        setSelectedProgramStageToDisplay(
            selectedProgramToDisplay.programStages.find(
                (p) => p.id === selected,
            ),
        );
        setSelectedDataElementDimensionToDisplay(null);
    ...
    const RenderDataElementContent = selectedProgramToDisplay && (
        <div className="mt-2">
            <div>
                {selectedProgramToDisplay.programStages?.length > 0 && (
                    <Field label="Program stage">
                        ...
                        >
                            {selectedProgramToDisplay.programStages.map(
                                (programStage) => (
                                    <SingleSelectOption
            ...
            {selectedProgramStageToDisplay &&
                selectedProgramStageToDisplay.programStageDataElements?.length >
                    0 && (
                    <div className="mt-2">
                        <Field label="Data Element">
                            <SingleSelect
                                ...
                                selected={
                                    selectedDataElementDimensionToDisplay?.id
                                }
                                onChange={({ selected }) =>
                                    setSelectedDataElementDimensionToDisplay(
                                        selectedProgramStageToDisplay.programStageDataElements
                            ...
                            >
                                {selectedProgramStageToDisplay.programStageDataElements.map(
                                    (stage) => (
                                        <SingleSelectOption
                                            key={stage.dataElement.id}
                                            value={stage.dataElement?.id}
                                            label={stage.dataElement?.name}
                                        />
                                    ),
                                )}
                            </SingleSelect>
            ...
            <div className="mt-2">
                <Field label="Which data ? ">
                    <SingleSelect
                        ...
                        onChange={({ selected }) =>
                            setSelectedEnrollmentDataToDisplay(selected)
                        }
                    >
                        <SingleSelectOption
                            key={ORGANISATION_UNIT_NAME}
                            value={ORGANISATION_UNIT_NAME}
    ...
    const handleInjectOtherOrganisationUnitElementToHTML = () => {
        if (
            selectedOtherOrganisationUnitElements.length > 0 &&
            currentHtmlTagSelected
        ) {
            const id_string = selectedOtherOrganisationUnitElements.join("|");

            const name_string = " ( "
                .concat(selectedOtherOrganisationUnitElements.join("-"))
                .concat(" ) ");
            ...
            window
                .$(currentHtmlTagSelected)
                .attr("data-is", ORGANISATION_UNIT_NAME);
            window.$(currentHtmlTagSelected).html("");
            window.$(currentHtmlTagSelected).html(name_string);
    ...
    const RenderOtherOrganisationUnitElementModal = () =>
        visibleOtherOrganisationUnitElementModal ? (
            <Modal onClose={handleCloseOtherOrganisationUnitElementModal} small>
                <ModalTitle>Organisation Unit name</ModalTitle>
                <ModalContent>
                        ...
                        {getParentObjectList().map((o) => (
                            <div key={o.value}>
                                <Checkbox
                                    ...
                                    onChange={({ value }) => {
                                        if (
                                            selectedOtherOrganisationUnitElements.includes(
                                                value,
                                            )
                                        ) {
                                            setSelectedOtherOrganisationUnitElements(
                                                selectedOtherOrganisationUnitElements.filter(
                                                    (o) => o !== value,
                                                ),
                                            );
                                        } else {
                ...
                <ModalActions>
                    <ButtonStrip end>
                        <Button
                            onClick={
                                handleCloseOtherOrganisationUnitElementModal
                            }
                        >
                            close
                        </Button>
                        <Button
                            ...
                            // disabled={}
                            onClick={() =>
                                handleInjectOtherOrganisationUnitElementToHTML()
                            }
                        >
                            OK
                        </Button>
    ...
    const SelectDXTrackerModal = () =>
        visibleSelectDXTrackerModal ? (
            <Modal onClose={handleCloseSaveTrackerDimensionModal} small>
                <ModalTitle>Tracker Dimension</ModalTitle>
                <ModalContent>
                    <div className="border rounded p-3">
                        {RenderDataToUsed}
                ...
                <ModalActions>
                    <ButtonStrip end>
                        <Button
                            onClick={() =>
                                handleCloseSaveTrackerDimensionModal()
                            }
                        >
                            close
                        </Button>
    ...
    const handleChangeSelectedOuGroupToDisplay = ({ selected }) =>
        setSelectedOuGroupToDisplay(selected);

    const SelectDXAggregateModal = () =>
        visibleSelectDXAggregateModal ? (
            <Modal onClose={handleCloseSaveAggregateDimensionModal} small>
                <ModalTitle>Aggregate Dimension</ModalTitle>
                <ModalContent>
                    <div className="border rounded p-3">
                        <div>
                            <Field label="Select organisation unit level to display">
                                <SingleSelect
                                    ...
                                    selected={selectedOuLevelToDisplay}
                                    onChange={({ selected }) =>
                                        setSelectedOuLevelToDisplay(selected)
                                    }
                                >
                                    {getParentListFields()}
                                </SingleSelect>
                            ...
                            organisationUnitGroups.length > 0 && (
                                <div className="mt-2">
                                    <Field label="Select organisation unit group ( optional ) ">
                                        <SingleSelect
                                            placeholder="Organisation Unit group"
                                            selected={selectedOuGroupToDisplay}
                                            onChange={
                                                handleChangeSelectedOuGroupToDisplay
                                            }
                                        >
                                            {organisationUnitGroups.map(
                                                (ouG) => (
                                                    <SingleSelectOption
                                                        key={ouG.id}
                                                        value={ouG.id}
                                    ...
                                    <div className="mt-2">
                                        <Field label="Choose legend to display">
                                            <SingleSelect
                                                placeholder="Choose legend to display"
                                                selected={
                                                    selectedTypeLegendToDisplay
                                                }
                                                onChange={({ selected }) =>
                                                    setSelectedTypeLegendToDisplay(
                                                        selected,
                                                    )
                                                }
                                            >
                ...
                <ModalActions>
                    <ButtonStrip end>
                        <Button
                            onClick={() =>
                                handleCloseSaveAggregateDimensionModal()
                            }
                        >
                            close
                        </Button>
    ...
    const SelectDXTitleModal = () =>
        visibleSelectDXTitle ? (
            <Modal onClose={() => handleCloseSelectDXTitleModal()} small>
                <ModalTitle>Dimension Title</ModalTitle>
                <ModalContent>
                    <div className="border rounded p-3">
                        <div>
                            <Field label="Select organisation unit level to display">
    ...
    const RenderTrackerContifigurationDimensionContent = () => (
        <TrackerDimensionsDialog
            selectedPrograms={selectedTrackerConfigDimensionsPrograms}
            setSelectedPrograms={setSelectedTrackerConfigDimensionsPrograms}
        />
    );

    const RenderAggregateContifigurationDimensionContent = () => (
        <DataDimension
    ...
    const ConfigurationModal = () =>
        visibleConfigurationModal ? (
            <Modal onClose={() => handleCloseConfigurationModal()} large>
                    ...
                    <div style={{ position: "relative", overflow: "hidden" }}>
                        <div className="my-2">
                            <Segmented
                                ...
                                onChange={(value) =>
                                    setSelectedDimensionContentToDisplay(value)
                                }
                            />
                        </div>

                        {selectedDimensionContentToDisplay === TRACKER.value &&
                            RenderTrackerContifigurationDimensionContent()}
                        {selectedDimensionContentToDisplay ===
                            AGGREGATE.value &&
                            RenderAggregateContifigurationDimensionContent()}
                ...
                <ModalActions>
                    <ButtonStrip end>
                        <Button
                            ...
                            icon={<CgCloseO style={{ fontSize: "16px" }} />}
                        >
                            close
                        </Button>
                    </ButtonStrip>
            ...
            <div className="card bg-white rounded border my-shadow ">
                <div className="card-header py-1 bg-white">
                    ...
                    <span style={{ fontSize: "12px" }}>
                        {" ( " +
                            selectedTrackerConfigDimensionsPrograms.length +
                            " )"}{" "}
                    </span>
                </div>
                            ...
                            {selectedTrackerConfigDimensionsPrograms.length >
                                ...
                                <div>
                                    {selectedTrackerConfigDimensionsPrograms
                                        .filter((dim) =>
                                            searchInAllInput &&
                                            searchInAllInput?.trim()?.length > 0
                                                ? dim.name
                                                      ?.toLowerCase()
                                                      ?.includes(
                                                          searchInAllInput?.toLowerCase(),
                                                      )
                                                : true,
                                        )
                                        .map((prog) => (
        ...
        <div className="mt-2">
            <div className="card bg-white rounded border my-shadow ">
                <div className="card-header py-1 bg-white">
                    ...
                    <span style={{ fontSize: "12px" }}>
                        {" ( " +
                            selectedConfigDimensionsAggregate.filter(
                                (dim) => dim.type === DATA_SET_EVENT,
                            ).length +
                            " )"}{" "}
                    </span>
                </div>
                <div className="collapse" id="dataSetEventCollapse">
                    <div className="card-body">
                        ...
                        >
                            {selectedConfigDimensionsAggregate.filter(
                                (dim) => dim.type === DATA_SET_EVENT,
                            ).length === 0 && <div> No Data set event </div>}
                            {selectedConfigDimensionsAggregate.filter(
                                (dim) => dim.type === DATA_SET_EVENT,
                            ).length > 0 && (
                                <div>
                                    {selectedConfigDimensionsAggregate
                                        .filter((dim) =>
                                            dim.type === DATA_SET_EVENT
                                                ? searchInAllInput &&
                                                  searchInAllInput.trim()
                                                      ?.length > 0
                                                    ? dim?.name
                                                          ?.toLowerCase()
                                                          ?.includes(
                                        ...
                                        .map((dimension) => (
                                            <div
                                                ...
                                                key={dimension.id}
                                                onClick={() =>
                                                    handleClickOnAggregateDimensionElement(
                                                        dimension,
                                                    )
                                                }
                                            >
                ...
                <div className="collapse" id="indicatorCollapse">
                    <div className="card-body">
                        ...
                        >
                            {selectedConfigDimensionsAggregate.filter(
                                (dim) => dim.type === INDICATOR,
                            ).length === 0 && <div> No Indicators </div>}
                            {selectedConfigDimensionsAggregate.filter(
                                (dim) => dim.type === INDICATOR,
                            ).length > 0 && (
                                <div>
                    ...
                    <div className="card-body">
                        ...
                        >
                            {selectedConfigDimensionsAggregate.filter(
                                ...
                                <div> No Category option combo </div>
                            )}
                            {selectedConfigDimensionsAggregate.filter(
                                (dim) => dim.type === CATEGORY_COMBO,
                            ).length > 0 && (
                ...
                <div className="collapse" id="dataElementCollapse">
                    <div className="card-body">
                        ...
                        >
                            {selectedConfigDimensionsAggregate.filter(
                                (dim) => dim.type === DATA_ELEMENT,
                            ).length === 0 && <div> No Data Element </div>}
                            {selectedConfigDimensionsAggregate.filter(
                                (dim) => dim.type === DATA_ELEMENT,
                            ).length > 0 && (
                                <div>
            ...
            <div className="card bg-white rounded border my-shadow ">
                <div className="card-header py-1 bg-white">
                    ...
                    <span style={{ fontSize: "12px" }}>
                        {" ( " +
                            selectedConfigDimensionsAggregate.filter(
                                (dim) => dim.type === PROGRAM_INDICATOR,
                            ).length +
                            " )"}{" "}
                    </span>
                </div>
                    ...
                    <div className="card-body">
                        ...
                        >
                            <div
                                style={{ fontSize: "14px" }}
                                className="my-dx"
                                onClick={handleClickOtherOrganisationUnitName}
                            >
                                {" "}
                            ...
                            <div
                                style={{ fontSize: "14px" }}
                                className="my-dx"
                                onClick={handleClickOtherElementDate}
                            >
                                {" "}
                                Date{" "}
                            </div>
    ...
    const RenderAddReport = () =>
        loadingInitState ? (
            ...
            <div style={{ width: "100%" }}>
                <div
                    className="my-shadow p-3 bg-white"
                    style={{ position: "sticky", top: "0px", zIndex: 100 }}
                >
                    {editReport && currentRepport ? (
                ...
                <div className="row py-4 px-3">
                    {editReport && currentRepport && !currentRepportContent && (
                        <NoticeBox
                            warning
                        >{`${currentRepport?.name} Content not found !`}</NoticeBox>
                    )}
                    ...
                    <div className="col-md-3">
                        <div style={{ position: "sticky", top: 30 }}>
                                ...
                                <div style={{ margin: "20px 0px" }}>
                                    ...
                                    >
                                        <Input
                                            ...
                                            value={searchInAllInput}
                                            placeholder="Filter in all elements"
                                        />
                                        <Button
                                            icon={
                                                <AiFillCloseCircle
                            ...
                            {selectedDimensionContentToDisplay ===
                                AGGREGATE.value &&
                                RenderAggregateDimensionContent()}
                            {selectedDimensionContentToDisplay ===
                                TRACKER.value &&
                                RenderTrackerDimensionContent()}
                        </div>
                    </div>
                </div>
            </div>
    ...
    const handleDeleteReport = async (id) => {
        try {
            if (id) {
                ...
                await deleteKeyFromDataStore(baseUrl, `REPORT_${id}`);
                loadDataStore(
                    baseUrl,
                    process.env.REACT_APP_REPORTS_KEY,
                    setLoadingReports,
    ...
    const handleEditReport = async (report) => {
        try {
            ...
            setEditReport(true);
            const content = await loadDataStore(
                baseUrl,
                `REPORT_${report.id}`,
                setLoadingInitState,
                null,
            ...
            if (!content) throw new Error(report.name + " Content not found !");
            ...
            <div className="p-3">
                <div className="bg-white p-2 rounded my-shadow">
                    <div className="text-right">
                        <Button primary onClick={handleAddReport}>
                            New Report
                        </Button>
                    </div>
                    <div className="mt-1">
